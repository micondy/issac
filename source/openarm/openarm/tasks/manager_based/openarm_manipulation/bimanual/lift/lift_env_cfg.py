...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    mini_table = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=[0.50, 0.0, 0.28], rot=[0.707, 0, 0, 0.707]),
        spawn=UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/SeattleLabTable/table_instanceable.usd",
            scale=(0.95, 1.05, 0.60),
        ),
    )

    spawn_region_left = AssetBaseCfg(
        prim_path="{ENV_REGEX_NS}/SpawnRegionLeft",
        init_state=AssetBaseCfg.InitialStateCfg(pos=[0.175, 0.33, 0.305]),
...
class EventCfg:
    ...
    reset_left_object_position = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (-0.475, 0.025), "y": (-0.07, 0.43), "z": (0.22, 0.28)},
            "velocity_range": {},
...
@configclass
class ObjectTableRightSceneCfg(InteractiveSceneCfg):
    """Configuration for right-only lift scene with robot, table and one cube."""
...
@configclass
class CommandsRightOnlyCfg:
    """Right-only command terms for the MDP."""
...
@configclass
class ActionsRightOnlyCfg:
    """Right-only action specifications for the MDP."""
...
@configclass
class ObservationsRightOnlyCfg:
    """Right-only observation specifications for the MDP."""
...
@configclass
class EventRightOnlyCfg:
    """Configuration for right-only events."""
...
@configclass
class RewardsRightOnlyCfg:
    """Right-only reward terms for the MDP."""
...
@configclass
class TerminationsRightOnlyCfg:
    """Right-only termination terms for the MDP."""
...
@configclass
class CurriculumRightOnlyCfg:
    """Right-only curriculum terms for the MDP."""
...
@configclass
class LiftBiRightOnlyEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the right-only lifting environment on bimanual robot."""

    scene: ObjectTableRightSceneCfg = ObjectTableRightSceneCfg(num_envs=4096, env_spacing=2.5)
    observations: ObservationsRightOnlyCfg = ObservationsRightOnlyCfg()
    actions: ActionsRightOnlyCfg = ActionsRightOnlyCfg()
    commands: CommandsRightOnlyCfg = CommandsRightOnlyCfg()
    rewards: RewardsRightOnlyCfg = RewardsRightOnlyCfg()
    terminations: TerminationsRightOnlyCfg = TerminationsRightOnlyCfg()
    events: EventRightOnlyCfg = EventRightOnlyCfg()
    curriculum: CurriculumRightOnlyCfg = CurriculumRightOnlyCfg()
